...
pub const GETHEADERS: [u8; 12] = [
    0x67, 0x65, 0x74, 0x68, 0x65, 0x61, 0x64, 0x65, 0x72, 0x73, 0x00, 0x00,
];

pub const HEADERS: [u8; 12] = [
    0x68, 0x65, 0x61, 0x64, 0x65, 0x72, 0x73, 0x00, 0x00, 0x00, 0x00, 0x00,
];
...
impl Serialize for IpAddress {
    ...
    fn deserialize(bytes: &mut &[u8]) -> Result<Self, P2PError> {
        ...
        let service =
            u64::from_le_bytes(service.try_into().map_err(|_| {
                P2PError::Parse(format!("Error while try to convert bytes into u64"))
            })?);
        ...
        let port =
            u16::from_be_bytes(port.try_into().map_err(|_| {
