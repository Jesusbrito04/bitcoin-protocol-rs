...
use crate::{P2PError, Serialize, network::IpAddress};
...
impl PeerStore {
    pub fn new() -> Result<Self, P2PError> {
        let db = open(BASE_PATH).map_err(|_| P2PError::Custom("Error opening database".to_string()) )?;
        if let Some(version) = db.get(b"__version__").map_err(|_| P2PError::Custom("Not version db found".to_string()))? {
            if version != CURRENT_DB_VERSION.to_le_bytes() {
            return Err(P2PError::Custom("Incompatible db version".to_string()));
        };
        }
        let _ = db.insert(b"__version__", &CURRENT_DB_VERSION.to_le_bytes()).map_err(|_| "Error inserting db version");
        Ok(PeerStore {
    ...
    pub fn add_peer(&self, value: IpAddress) -> Result<Option<IVec>, P2PError> {
        ...
        self.db.insert(key, value).map_err(|_| P2PError::Custom("Error to insert peer".to_string()))
    }
    pub fn get_peers(&self) -> Result<Vec<IpAddress>, P2PError> {
        let mut peers: Vec<IpAddress> = Vec::new();
        for item in self.db.iter() {
            let (key, value) = item.map_err(|_| P2PError::Custom("Error to get peer".to_string()))?;
