...
impl VersionMessage {
    ...
    pub fn deserialize(mut bytes: &[u8]) -> Result<Self, P2PError> {
        if bytes.len() < 4 + 8 + 8 {
            return Err(P2PError::InvalidVersionMessageLen);
        }
        let version = i32::from_le_bytes(bytes[0..4].try_into().map_err(|_| {
            P2PError::ConvertionError(format!("Error while try to convert bytes into i32"))
        })?);
        ...
        let timestamp = i64::from_le_bytes(bytes[12..20].try_into().map_err(|_| {
            P2PError::ConvertionError(format!("Error while try to convert bytes into i64"))
        })?);
        bytes = &bytes[20..];
        ...
        let addr_recv_ip: [u8; 16] = bytes[8..24]
            .try_into()
            .map_err(|err| P2PError::ParseError(format!("{}", err)))?;
        ...
        let user_agent = String::from_utf8(ua_slice.to_vec()).map_err(|_| {
            P2PError::ConvertionError(format!("Error while try to convert bytes into String"))
        })?;
        bytes = rest;
